...
class BotAdmin(BaseModel, PersistableMixin):  # pylint: disable=too-few-public-methods
    """List of bots a user is administering"""

    __tablename__ = "telegram_bot_admin"
    bot_id = db.Column(
        BigIntegerType,
    ...
    bot = db.relationship("TelegramBot", back_populates="admins", uselist=False)
    ...
    user = db.relationship("TelegramUser", back_populates="bots_admin", uselist=False)

    active = db.Column(db.Boolean, server_default=expression.true(), nullable=False)


BaseModel.BotAdmin = BotAdmin
